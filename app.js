...
function getHealthSyncSetupHTML() {
  ...
  return `<div class="health-setup">
    <div style="margin-bottom:14px">
      <button class="btn-secondary" style="width:100%" onclick="MQ.refreshHealthSync()">🔄 Refresh Health Data</button>
      <p style="color:var(--text-muted);font-size:11px;margin:5px 0 0;text-align:center">Run your Shortcut first, then tap this to pull the latest data</p>
    </div>
    ...
    <div class="health-setup-steps">
      ...
      <div class="setup-step"><span class="step-num">7</span><span>Add <strong>"Text"</strong> action and paste this (tap to copy), then replace each value with its magic variable pill:</span></div>
    </div>
    <div class="setup-url-box" onclick="MQ.copyHealthJSON()">
      <code class="setup-url" style="font-size:10px">{"fields":{"date":{"stringValue":"myDate"},"steps":{"doubleValue":mySteps},"protein":{"doubleValue":myProtein},"activeCalories":{"doubleValue":myActiveCal},"sleepHours":{"doubleValue":mySleepHours},"exerciseMinutes":{"doubleValue":0}}}</code>
    ...
    <div class="health-setup-steps">
      <div class="setup-step"><span class="step-num">8</span><span>Add <strong>"Get Contents of URL"</strong>:<br>
        ...
        • Request Body: tap it → choose <strong>File</strong> → insert the <em>Text</em> variable from step 7<br>
        ⚠️ Do <strong>not</strong> use JSON body type — it will break the format
      </span></div>
...
function renderHomeGym() {
  ...
  const avatarSVG = renderMiniAvatarSVG(gender, cosmetics, 0, state.name || 'You', state.hair || (gender === 'female' ? 'space_buns' : 'default'), state.skinTone ?? 1);
...
function renderMiniAvatarSVG(gender, cosmetics, idx, playerName, hairStyle, skinToneIdx) {
  ...
  podium.innerHTML = `<div class="lb-podium">
    ${[1, 0, 2].map(pos => {
      ...
      return `<div class="podium-slot rank-${rank}${p.you ? ' podium-you' : ''}">
        <div class="podium-avatar">${renderMiniAvatarSVG(p.gender || 'male', p.cosmetics || [], pos, p.name || '', p.hair || (p.gender === 'female' ? 'space_buns' : 'default'), p._skinTone ?? 1)}</div>
...
async function testHealthSync() {
  ...
  const body = JSON.stringify({ fields: {
    ...
    steps:           { doubleValue: 9500 },
    protein:         { doubleValue: 145 },
    activeCalories:  { doubleValue: 420 },
    sleepHours:      { doubleValue: 7.5 },
    exerciseMinutes: { doubleValue: 45 },
  }});
  try {
    toast('Sending test data…');
    const res = await fetch(url, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    await loadHealthSync();
    toast('Test sync worked! ✅');
  } catch(e) {
...
return {
  ...
  login, logout, toggleCosmetic, selectPose, toggleSettings, toast, syncStateFromStorage, setSkinTone, copyHealthJSON, testHealthSync, refreshHealthSync,
