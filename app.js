...
async function login() {
  ...
  if (db) {
    try {
      ...
      state = defaultState();
      state.name = username;
      hint.textContent = 'Offline mode — data saved locally';
...
function submitWorkout() {
  ...
  for (const p of parsed) {
    ...
    if (p.isRest) {
      totalXP = 100;
      const restXPPer = Math.floor(100 / 10);
...
function renderBodyCosmeticsSVG(front, fLX, fRX, fW, shortsX, shortsW, torsoX, torsoW) {
  ...
  if (equipped.includes('wrist_wraps')) {
    svg += `<rect x="${fLX}" y="168" width="${fW}" height="8" rx="3" fill="#e0e0e0"/>`;
    svg += `<rect x="${fRX}" y="168" width="${fW}" height="8" rx="3" fill="#e0e0e0"/>`;
  }
  if (equipped.includes('wrist_wraps_red')) {
    svg += `<rect x="${fLX}" y="168" width="${fW}" height="8" rx="3" fill="#e53935"/>`;
    svg += `<rect x="${fRX}" y="168" width="${fW}" height="8" rx="3" fill="#e53935"/>`;
  }
  if (equipped.includes('sweatbands')) {
    svg += `<rect x="${fLX}" y="168" width="${fW}" height="8" rx="3" fill="#42a5f5"/>`;
    svg += `<rect x="${fRX}" y="168" width="${fW}" height="8" rx="3" fill="#42a5f5"/>`;
    svg += `<line x1="${fLX+2}" y1="172" x2="${fLX+fW-2}" y2="172" stroke="#1e88e5" stroke-width="0.8"/>`;
    svg += `<line x1="${fRX+2}" y1="172" x2="${fRX+fW-2}" y2="172" stroke="#1e88e5" stroke-width="0.8"/>`;
  }
...
// ─── Friends Feed (mock data, Firebase-ready) ───
const MOCK_FRIENDS = [
  { name: 'DragonSlayer99', initials: 'DS', color: '#f44336' },
  { name: 'IronMaiden',     initials: 'IM', color: '#9c27b0' },
  { name: 'GymReaper',      initials: 'GR', color: '#4caf50' },
  { name: 'FlexGordon',     initials: 'FG', color: '#ff9800' },
  { name: 'SwoleMaster',    initials: 'SM', color: '#2196f3' },
];

function generateMockFriendActivity() {
  ...
  for (let d = 0; d < 5; d++) {
    ...
    for (let f = 0; f < friendCount; f++) {
      const friend = shuffled[f];
      const numMuscles = 1 + Math.floor(Math.random() * 3);
      const trained = [];
      for (let m = 0; m < numMuscles; m++) {
        const mk = muscleKeys[Math.floor(Math.random() * muscleKeys.length)];
        if (!trained.includes(mk)) trained.push(mk);
      }
      const xp = 8 + Math.floor(Math.random() * 45);
      activities.push({
        friend,
        muscles: trained,
        xp,
...
function renderFriendsFeed() {
  ...
  const days = generateMockFriendActivity();

  if (!days.length) {
    container.innerHTML = '<p class="muted" style="text-align:center;padding:32px">No friend activity yet.</p>';
    return;
  }

  container.innerHTML = days.map(day => `
    <div class="friend-day">
      <div class="friend-date">${formatDate(day.date + 'T12:00:00')}</div>
      ${day.activities.map(a => `
        <div class="friend-activity">
          <div class="friend-avatar" style="background:${a.friend.color}22;color:${a.friend.color}">${a.friend.initials}</div>
          <div class="friend-activity-info">
            <div class="friend-name">${a.friend.name}</div>
            <div class="friend-summary">Trained ${a.muscles.map(m => MUSCLES[m].name).join(', ')}</div>
          </div>
...
async function renderLeaderboard(type) {
  ...
  if (!players) {
    players = [
      { name: 'DragonSlayer99', xp: 12400 },
      { name: 'IronMaiden', xp: 9800 },
      { name: 'GymReaper', xp: 7600 },
      { name: state.name, xp: myXP, you: true },
      { name: 'FlexGordon', xp: 4200 },
      { name: 'SwoleMaster', xp: 3100 },
      { name: 'GainzVille', xp: 1800 },
    ].sort((a, b) => b.xp - a.xp);
  }
