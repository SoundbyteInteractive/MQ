...
// ─── Version & Patch Notes ───
// Bump the LAST number by +1 on every push. Bump the middle number on a
// significant addition (new cosmetics/armor sets, bosses, features, or a
// rework of an existing ruleset). Add a matching entry to PATCH_NOTES, newest first.
...
const COSMETICS = [
  ...
  { id: 'raid_trophy',     name: 'Raid Trophy',    icon: '🏆', type: 'neck',  unlock: 'raid_slayer',   desc: 'Defeat a monthly raid boss' },
  ...
  { id: 'gym_towel',       name: 'Microfibre Towel', icon: '<i class="ti ti-stretching-2"></i>', type: 'neck', unlock: null, desc: 'Purchased from the Iron Depot' },
  ...
  { id: 'sunglasses',     name: 'Aviator Shades',     icon: '<i class="ti ti-sunglasses"></i>', type: 'head',  price: 70,  desc: "Can't see my eyes, can't see my gains" },
  { id: 'arm_sleeve',     name: 'Compression Sleeve', icon: '<i class="ti ti-bandage"></i>',     type: 'wrist', price: 55,  desc: 'Single-arm sleeve — for the aesthetic' },
  { id: 'champ_cape',     name: "Champion's Cape",    icon: '🦸', type: 'accessory', price: 220, desc: 'Nobody asked, everybody stares' },
  { id: 'dog_tags',       name: 'Dog Tags',           icon: '<i class="ti ti-tag"></i>',        type: 'neck',   price: 40,  desc: 'Budget bling — still counts' },
];
...
function _maybeDropArmor(raidId, raidSize) {
  ...
  const pieceDef = ARMOR_SETS[setId].pieces[slot];
  const rarityInfo = ARMOR_RARITY[rarity];
  toast(`${variant ? '✨ ' : ''}${rarityInfo.label} drop: ${variant ? 'Bloodforged ' : ''}${pieceDef.name}!`, 'success');
}
...
async function login() {
  ...
  if (!usedFirebase) {
    ...
    if (raw) {
      ...
      state = defaultState();
      state.name = username;
      localStorage.setItem(savedKey, JSON.stringify({ ...state, _pin: hashedPin }));
...
async function autoLogin() {
  ...
  try {
    ...
    const data = doc.data();
    if (data._pin && data._pin !== pin && data._pin !== legacyPin) { logout(); return; }
...
function saveWithPin() {
  ...
  const leaderboardData = {
    ...
    _totalWorkouts: state.totalWorkouts || 0,
    _currentStreak: state.streak || 0,
    _activeTitle: state.activeTitle || null,
    _private: state.private || false,
    _raidDamage: state.raidDamage || 0,
    _raidWeekStart: state.raidWeekStart || '',
    _teamId: state.teamId || null,
    _gymId: state.gymId || null,
    _muscleLevels: Object.fromEntries(
      Object.keys(MUSCLES).filter(k => k !== 'rest' && k !== 'cardio')
        .map(k => [k, state.muscles?.[k]?.level || 1])
    ),
  };
  localStorage.setItem('musclequest_save', JSON.stringify(state));
  if (currentUser && !db) {
...
function updateStreak(targetDate) {
  ...
  if (date === today) {
    ...
    if (state.lastWorkoutDate === yesterday) {
      ...
      else { state.streak++; _streakPulsePending = true; }
    }
...
function submitWorkout() {
  ...
  if (state.teamId && state.teamRole) {
    _applyTeamWorkoutEffects(teamRaidContribution, grandTotalXP).catch(() => {});
  }
...
function showTab(tab) {
  ...
  if (currentTab === 'settings' && tab !== 'settings') _previewActive = null;
  ...
  if (tab === 'dashboard') renderDashboard();
    ...
    const privLabel = document.getElementById('private-toggle-label');
    if (privLabel) privLabel.textContent = state.private ? 'On' : 'Off';
    renderCosmeticsList();
    renderTitlesSection();
...
function renderHeadCosmeticsSVG(front) {
  ...
  const headArmor = _equippedArmorPiece('head');
...
function renderWristCosmeticSVG(fx, fw, isRight) {
  const equipped = state.equippedCosmetics || [];
  const wristArmor = _equippedArmorPiece('wrist');
...
function renderBodyCosmeticsSVG(front, fLX, fRX, fW, shortsX, shortsW, torsoX, torsoW, calfLX, calfRX, calfW) {
  const equipped = state.equippedCosmetics || [];
  const hasAnyArmor = ARMOR_SLOTS.some(s => _equippedArmorPiece(s));
  ...
  const torsoArmor = _equippedArmorPiece('torso');
  if (torsoArmor) {
  ...
  const waistArmor = _equippedArmorPiece('waist');
  if (waistArmor) {
  ...
  const legsArmor = _equippedArmorPiece('legs');
...
async function renderGuildHall() {
  ...
  container.innerHTML = `
    <div class="store-back-bar">
      <button class="store-back-btn" onclick="GainsShop.openTrainer()">← Iron Temple</button>
...
function _renderGymRosterScreen(container, name, motto, members, ownerUsername) {
  ...
  const rowsHTML = sorted.map(m => {
    ...
    return `<div style="display:flex;align-items:center;gap:10px;padding:9px 10px;border-bottom:1px solid #3a2000;background:#150a02">
      ...
      ${iCanManageThem ? `
        <select onchange="MQ.setGymMemberRank('${m.username}', this.value)" style="font-size:11px;padding:5px 6px;border-radius:6px;border:1px solid #5a3418;background:#0d0500;color:#e8d8b8">${rankOptions}</select>
...
async function _applyTeamWorkoutEffects(raidContribution, sessionXP) {
  ...
  await db.runTransaction(async tx => {
    ...
    tx.update(ref, { members, actionLog: log, bossDamage, bossDefeated });
  });
}
...
function renderPartyCard(el, teamName, members, ownerUsername, actionLog, raidName, raidSize, gymRoster) {
  ...
  const gymInviteHTML = (gymRoster && gymRoster.length) ? `
    <div style="display:flex;gap:6px;margin-bottom:8px">
      <select id="team-gym-invite-select" style="flex:1;padding:8px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text);font-size:13px">
        ${gymRoster.map(m => `<option value="${esc(m.username)}">${esc(m.name || m.username)} (Lv.${m.level || 1})</option>`).join('')}
      </select>
      <button onclick="MQ.inviteGymMemberToTeam()" style="padding:8px 14px;border-radius:8px;border:1px solid var(--accent,#7c4dff);background:none;color:var(--accent,#7c4dff);font-size:13px;font-weight:600;cursor:pointer">Invite from Guild</button>
    </div>` : '';
  ...
  el.innerHTML = `
    ...
    ${logHTML}
    ${!isFull ? gymInviteHTML : ''}
    ${!isFull ? `<div style="display:flex;gap:6px;margin-bottom:8px">
      <input id="team-invite-input" placeholder="Invite by username" style="flex:1;padding:8px 10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text);font-size:13px;box-sizing:border-box">
      <button onclick="MQ.inviteToTeam()" style="padding:8px 14px;border-radius:8px;border:none;background:var(--accent,#7c4dff);color:#fff;font-size:13px;font-weight:600;cursor:pointer">Invite</button>
    </div>` : ''}
...
function inviteGymMemberToTeam() {
  const username = document.getElementById('team-gym-invite-select')?.value;
  if (!username) { toast('No guild members available to invite'); return; }
  inviteToTeam(username);
}
...
function togglePrivate(on) {
  state.private = !!on;
  saveWithPin();
  const label = document.getElementById('private-toggle-label');
  if (label) label.textContent = state.private ? 'On' : 'Off';
...
// Regular cosmetics + raid armor now live in one unified list. Armor is shown behind a
// tab strip — one set's 5 pieces visible at a time — instead of dumping every owned
// piece from every set into one giant grid, which gets unreadable fast once you're
// sitting on 2-3 sets' worth of drops.
let _armorSetTab = null;
function selectArmorSetTab(setId) {
  _armorSetTab = setId;
  renderCosmeticsList();
}

// Category filter — one chip per wearable slot, so a big collection doesn't turn into
// an unsorted wall of tiles. Categories are derived from the `type` field every regular
// cosmetic already carries, which happens to be the same vocabulary armor pieces use
// for `slot`, so one filter covers both without any extra mapping.
const COSMETIC_CATEGORY_LABELS = { head: 'Head', neck: 'Neck', torso: 'Torso', wrist: 'Wrist', waist: 'Waist', legs: 'Legs', feet: 'Feet', body: 'Body', accessory: 'Accessory' };
let _cosmeticsFilter = 'all';
function setCosmeticsFilter(type) {
  _cosmeticsFilter = type;
  renderCosmeticsList();
}

// Live preview — tapping an unequipped item renders it on a small avatar right in the
// Cosmetics panel (no more tab-switch-and-check), with an explicit Equip/Cancel action
// instead of a bare double-tap, so there's no risk of accidentally equipping something.
let _previewActive = null; // { kind: 'cosmetic'|'armor', id }

function previewCosmetic(id) {
  const cosmetic = COSMETICS.find(c => c.id === id);
  if (!cosmetic) return;
  _previewActive = { kind: 'cosmetic', id };
  renderCosmeticsList(); // renders the preview avatar too — see tail of that function
}

function previewArmor(instanceId) {
  const piece = (state.armorInventory || []).find(p => p.id === instanceId);
  if (!piece) return;
  _previewActive = { kind: 'armor', id: instanceId };
  renderCosmeticsList(); // renders the preview avatar too — see tail of that function
}
...
function confirmPreview() {
  ...
  const { kind, id } = _previewActive;
  _previewActive = null;
  if (kind === 'cosmetic') toggleCosmetic(id);
  else equipArmor(id);
}

// Placed inline right above whichever grid (regular cosmetics or armor) the previewed
// item belongs to, instead of one shared panel pinned to the top — keeps it next to
// what you're actually looking at instead of forcing a scroll back up to see it.
function _previewPanelHTML() {
  return `
    <div class="cosmetic-preview-panel">
      <div class="cosmetic-preview-avatar-wrap"><div id="cosmetics-preview-avatar" class="avatar-container"></div></div>
      <div class="cosmetic-preview-actions">
        <div style="font-size:11px;color:var(--muted);margin-bottom:8px">👁 Previewing — nothing's equipped yet</div>
        <div style="display:flex;gap:8px">
          <button class="btn-add-exercise" style="flex:1" onclick="MQ.clearPreview()">Cancel</button>
          <button class="btn-primary" style="flex:1" onclick="MQ.confirmPreview()">✓ Equip</button>
        </div>
      </div>
    </div>`;
}
...
function renderCosmeticsList() {
  ...
  if (!container) return;
  const purchased = state.purchasedCosmetics || [];
  const categoriesPresent = [...new Set(COSMETICS.map(c => c.type))];
  const filterChipsHTML = ['all', ...categoriesPresent].map(cat => {
    const active = _cosmeticsFilter === cat;
    return `<button onclick="MQ.setCosmeticsFilter('${cat}')" style="padding:5px 10px;border-radius:999px;border:1px solid ${active ? 'var(--accent,#7c4dff)' : 'var(--border)'};background:${active ? 'var(--accent,#7c4dff)22' : 'none'};color:${active ? 'var(--text)' : 'var(--muted)'};font-size:10.5px;font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0">${cat === 'all' ? 'All' : COSMETIC_CATEGORY_LABELS[cat] || cat}</button>`;
  }).join('');

  const cosmeticsHTML = COSMETICS.filter(c => _cosmeticsFilter === 'all' || c.type === _cosmeticsFilter).map(c => {
    ...
    const previewing = _previewActive?.kind === 'cosmetic' && _previewActive.id === c.id;
    const onclick = !unlocked ? '' : equipped ? `MQ.toggleCosmetic('${c.id}')` : `MQ.previewCosmetic('${c.id}')`;
    const reqText = !unlocked ? c.desc : equipped ? 'Equipped' : previewing ? '👁 Previewing' : 'Tap to preview';
    return `<div class="cosmetic-item ${unlocked ? (equipped ? 'equipped' : previewing ? 'previewing' : '') : 'locked'}"
      onclick="${onclick}">
      <span class="cosmetic-icon">${c.icon}</span>
      <span class="cosmetic-name">${c.name}</span>
      <span class="cosmetic-req">${reqText}</span>
    </div>`;
  }).join('');

  const inv = state.armorInventory || [];
  const equippedArmor = state.equippedArmor || {};
  let armorHTML = '';
  if (inv.length) {
    const bySet = {};
    for (const p of inv) (bySet[p.setId] = bySet[p.setId] || []).push(p);
    const ownedSetIds = Object.keys(bySet);
    if (!_armorSetTab || !ownedSetIds.includes(_armorSetTab)) _armorSetTab = ownedSetIds[0];

    const tabsHTML = ownedSetIds.map(setId => {
      const set = ARMOR_SETS[setId];
      const setColor = ARMOR_SET_COLORS[set.theme].accent;
      const eqCount = ARMOR_SLOTS.filter(s => bySet[setId].some(p => p.id === equippedArmor[s])).length;
      const active = setId === _armorSetTab;
      return `<button onclick="MQ.selectArmorSetTab('${setId}')" style="padding:6px 10px;border-radius:8px;border:1.5px solid ${active ? setColor : 'var(--border)'};background:${active ? setColor + '22' : 'none'};color:${active ? 'var(--text)' : 'var(--muted)'};font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;flex-shrink:0">${esc(set.name)}${eqCount > 0 ? ` <span style="opacity:.75">${eqCount}/5</span>` : ''}</button>`;
    }).join('');

    const activeSet = ARMOR_SETS[_armorSetTab];
    const activePieces = bySet[_armorSetTab];
    const equippedCount = ARMOR_SLOTS.filter(s => activePieces.some(p => p.id === equippedArmor[s])).length;
    const visiblePieces = _cosmeticsFilter === 'all' ? activePieces : activePieces.filter(p => p.slot === _cosmeticsFilter);
    const tiles = visiblePieces.length ? [...visiblePieces].sort((a, b) => ARMOR_RARITY[b.rarity].order - ARMOR_RARITY[a.rarity].order).map(p => {
      const isEquipped = equippedArmor[p.slot] === p.id;
      const isPreviewing = _previewActive?.kind === 'armor' && _previewActive.id === p.id;
      const rarityInfo = ARMOR_RARITY[p.rarity];
      const tileColor = p.variant ? '#c0304a' : rarityInfo.color;
      const label = (p.variant ? 'Bloodforged ' : '') + activeSet.pieces[p.slot].name;
      // Equipped pieces get a filled color background + bright border + glow — the old
      // version only changed a line of small text at the bottom, easy to miss since the
      // inline border-color (for rarity) was overriding the CSS .equipped border anyway.
      const equippedStyle = isEquipped ? `background:${tileColor}33;border-color:${tileColor};border-width:2px;box-shadow:0 0 14px ${tileColor}66;`
        : isPreviewing ? `border-color:var(--accent-glow);border-width:2px;box-shadow:0 0 14px var(--accent-glow, #7c4dff)66;`
        : `border-color:${tileColor}88;`;
      const onclick = isEquipped ? `unequipArmorSlot('${p.slot}')` : `previewArmor('${p.id}')`;
      const reqText = isEquipped ? '✓ EQUIPPED' : isPreviewing ? '👁 Previewing' : rarityInfo.label;
      return `<div class="cosmetic-item ${isEquipped ? 'equipped' : isPreviewing ? 'previewing' : ''}" style="${equippedStyle}" onclick="MQ.${onclick}">
        <span class="cosmetic-icon" style="color:${tileColor}"><i class="ti ti-shield"></i></span>
        <span class="cosmetic-name" style="${isEquipped ? `color:${tileColor};font-weight:800;` : ''}">${esc(label)}</span>
        <span class="cosmetic-req" style="color:${tileColor};${isEquipped ? 'font-weight:700;' : ''}">${reqText}</span>
      </div>`;
    }).join('') : `<p class="muted" style="font-size:11px;padding:4px 0;grid-column:1/-1">No ${COSMETIC_CATEGORY_LABELS[_cosmeticsFilter] || _cosmeticsFilter} pieces in ${esc(activeSet.name)}.</p>`;
    const raidName = RAID_DEFS.find(r => r.id === activeSet.raidId)?.name || 'its raid';
    const progressLine = `<div style="font-size:10.5px;color:${equippedCount >= 5 ? '#7bc82e' : 'var(--muted)'};margin:8px 0 6px">
      ${equippedCount}/5 equipped${
        equippedCount >= 5 ? ' — full set bonus active! (+21% XP, +10% gold)'
        : equippedCount >= 3 ? ' — 3pc bonus active (+10% XP). Equip all 5 for more.'
        : ' — equip 3 pieces for a bonus.'
      }${equippedCount < 5 ? ` Defeat ${esc(raidName)} for more pieces.` : ''}
    </div>`;

    armorHTML = `
      <div style="margin-top:16px;padding-top:12px;border-top:1px solid var(--border)">
        <div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:8px">⚔️ Raid Armor</div>
        <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:2px">${tabsHTML}</div>
        ${progressLine}
        ${_previewActive?.kind === 'armor' ? _previewPanelHTML() : ''}
        <div class="cosmetics-grid">${tiles}</div>
      </div>`;
  }

  container.innerHTML = `<div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:6px;margin-bottom:8px">${filterChipsHTML}</div>${_previewActive?.kind === 'cosmetic' ? _previewPanelHTML() : ''}<div class="cosmetics-grid">${cosmeticsHTML}</div>${armorHTML}`;

  if (_previewActive) {
    const savedCosmetics = [...(state.equippedCosmetics || [])];
    const savedArmor = { ...(state.equippedArmor || {}) };
    if (_previewActive.kind === 'cosmetic') {
      const cosmetic = COSMETICS.find(c => c.id === _previewActive.id);
      if (cosmetic) {
        state.equippedCosmetics = (state.equippedCosmetics || []).filter(c => {
          const other = COSMETICS.find(co => co.id === c);
          return other && other.type !== cosmetic.type;
        });
        state.equippedCosmetics.push(_previewActive.id);
      }
    } else {
      const piece = (state.armorInventory || []).find(p => p.id === _previewActive.id);
      if (piece) state.equippedArmor = { ...savedArmor, [piece.slot]: _previewActive.id };
    }
    renderAvatar('cosmetics-preview-avatar');
    state.equippedCosmetics = savedCosmetics;
    state.equippedArmor = savedArmor;
  }
}
...
function renderTitlesSection() {
  ...
  if (!unlocked.length) {
    container.innerHTML = '<p class="muted" style="font-size:12px;padding:8px 0">Unlock achievements to earn titles you can display on your profile.</p>';
    return;
  }
  const noneOption = `<div class="cosmetic-item ${!state.activeTitle ? 'equipped' : ''}" onclick="MQ.setActiveTitle(null)">
    <span class="cosmetic-icon"><i class="ti ti-x"></i></span>
    <span class="cosmetic-name">No Title</span>
    <span class="cosmetic-req">${!state.activeTitle ? 'Equipped' : 'Tap to equip'}</span>
  </div>`;
  container.innerHTML = `<div class="cosmetics-grid">${noneOption}${unlocked.map(a => {
    const equipped = state.activeTitle === a.title;
    return `<div class="cosmetic-item ${equipped ? 'equipped' : ''}" onclick="MQ.setActiveTitle('${a.title.replace(/'/g,"\\'")}')">
      <span class="cosmetic-icon">${a.icon}</span>
      <span class="cosmetic-name">"${a.title}"</span>
      <span class="cosmetic-req">${equipped ? 'Equipped' : a.name}</span>
    </div>`;
  }).join('')}</div>`;
}
...
return {
  ...
  showVolume, adoptLegendSplit, adoptHomeSplit, deleteSplit, openSplitBuilder, _saveSplitBuilder, _sbAddDay, _sbRemoveDay, _sbChangePreset, _sbToggleEx, _sbToggleMuscle, renderSettingsSplits, cycleSplitDay, shareSplit, importSplitCode, _showImportCode, setActiveTitle, togglePrivate, showMeasurementGraph, _saveMeasurement, _deleteMeasurement, createTeam, inviteToTeam, inviteGymMemberToTeam, acceptTeamInvite, declineTeamInvite, leaveTeam, showQuestTab, fixMyRole, addGold, kickTeamMember, _kickConfirm, renderGuildHall, createGym, joinGym, leaveGym, setGymMemberRank, kickGymMember, _raidMapWalkAndSelect, _backToRaidMap, showRaidTooltip, hideRaidTooltip,
  ...
  renderMiniAvatarSVG, getHairSVG, _buildBackAvatarSVG, equipArmor, unequipArmorSlot, debugUnlockEverything, debugSetStreak, debugCheckTeam, debugRepairTeamBoss, selectArmorSetTab, cycleHomeCard, setCosmeticsFilter,
  openBackdateModal, backdateCalNav, selectBackdateDay, showPatchNotes,
  previewCosmetic, previewArmor, clearPreview, confirmPreview,
};
