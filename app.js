...
function updateStreak(targetDate) {
  ...
  // Protection buffs (multivitamin/streakbar) bridge a single-day gap — only
  // meaningful when logging today's workout, since that's when the buff is live.
  if (date === today) {
    ...
    if (hasProtection) {
      ...
      if (!dates.has(yesterday) && (dates.has(dayBefore) || (state.streakProtectedDates || []).includes(dayBefore))) {
        if (!state.streakProtectedDates) state.streakProtectedDates = [];
        if (!state.streakProtectedDates.includes(yesterday)) state.streakProtectedDates.push(yesterday);
      }
    }
  }
...
function _renderSplitDay(split, dayIdx) {
  ...
  nav.innerHTML = `
    <button onclick="MQ.cycleSplitDay(-1)" style="background:var(--bg-card);border:1px solid var(--border);color:var(--text);border-radius:50%;width:30px;height:30px;font-size:16px;cursor:pointer;line-height:1;flex-shrink:0">‹</button>
    <div style="flex:1;text-align:center;min-width:0">
      <div style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap">
        <span style="font-size:13px;font-weight:700;color:var(--text)">${day.label}</span>
        ${isToday ? '<span style="font-size:9px;font-weight:700;color:#4caf50;background:#4caf5022;padding:1px 6px;border-radius:8px;letter-spacing:.4px">TODAY</span>' : ''}
      </div>
...
function renderMiniAvatarSVG(gender, cosmetics, idx, playerName, hairStyle, skinToneIdx, hairColorIdx, tattooId, forcePose) {
  ...
  if (eq.includes('champ_cape')) {
    bodyCosmetic += front
      ? `<path d="M40 38 Q38 42 39 47" stroke="#7c1520" stroke-width="4" fill="none" stroke-linecap="round" opacity="0.9"/><path d="M60 38 Q62 42 61 47" stroke="#7c1520" stroke-width="4" fill="none" stroke-linecap="round" opacity="0.9"/>`
      : `<path d="M40 37 Q33 62 37 88 Q50 94 63 88 Q67 62 60 37 Q50 41 40 37Z" fill="#7c1520"/><path d="M40 37 Q33 62 37 88 Q50 94 63 88 Q67 62 60 37 Q50 41 40 37Z" fill="none" stroke="#ffd700" stroke-width="0.8" opacity="0.85"/>`;
  }
...
function renderCosmeticsList() {
  ...
  if (inv.length) {
    ...
    const tiles = visiblePieces.length ? [...visiblePieces].sort((a, b) => ARMOR_RARITY[b.rarity].order - ARMOR_RARITY[a.rarity].order).map(p => {
      ...
      const equippedStyle = isEquipped ? `background:${tileColor}33;border-color:${tileColor};border-width:2px;box-shadow:0 0 14px ${tileColor}66;` : `border-color:${tileColor}88;`;
      return `<div class="cosmetic-item ${isEquipped ? 'equipped' : ''}" style="${equippedStyle}" onclick="MQ.${isEquipped ? `unequipArmorSlot('${p.slot}')` : `equipArmor('${p.id}')`}">
        ...
        <span class="cosmetic-req" style="color:${tileColor};${isEquipped ? 'font-weight:700;' : ''}">${isEquipped ? '✓ EQUIPPED' : rarityInfo.label}</span>
      </div>`;
  ...
  container.innerHTML = `<div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:6px;margin-bottom:8px">${filterChipsHTML}</div><div class="cosmetics-grid">${cosmeticsHTML}</div>${armorHTML}`;
}
...
return {
  ...
  renderMiniAvatarSVG, getHairSVG, _buildBackAvatarSVG, equipArmor, unequipArmorSlot, debugUnlockEverything, selectArmorSetTab, cycleHomeCard, setCosmeticsFilter,
