...
function renderVolumeBody(muscleSets) {
  ...
  const frontOverlay = [
    op('shoulders') > 0 && `<ellipse cx="40" cy="39" rx="5" ry="5" fill="${col('shoulders')}" opacity="${op('shoulders').toFixed(2)}"/>
    ...
    op('biceps') > 0 && `<ellipse cx="36" cy="49" rx="5" ry="9" fill="${col('biceps')}" opacity="${op('biceps').toFixed(2)}"/>
  ...
  const backOverlay = [
    ...
    op('triceps') > 0 && `<ellipse cx="36" cy="49" rx="5" ry="9" fill="${col('triceps')}" opacity="${op('triceps').toFixed(2)}"/>
...
async function renderLeaderboard(type) {
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        players.push({
          ...
          totalWorkouts: d._totalWorkouts || 0,
          currentStreak: d._currentStreak || 0,
          muscleLevels: d._muscleLevels || {},
        });
      });
  ...
  players = players.map(p => p.you ? { ...p, xp: myPeriodXP, gender: state.gender, cosmetics: state.equippedCosmetics, hair: state.hair, skinTone: state.skinTone ?? 1, hairColor: state.hairColor ?? 0, totalWorkouts: state.totalWorkouts || 0, currentStreak: state.currentStreak || 0, muscleLevels: myMuscleLevels } : p);
  players.sort((a, b) => b.xp - a.xp);
  ...
  container.innerHTML = players.map((p, i) => {
    ...
    const pData = encodeURIComponent(JSON.stringify({ name: p.name, level: p.level, xp: p.xp, gender: p.gender, cosmetics: p.cosmetics, hair: p.hair, skinTone: p.skinTone ?? 1, hairColor: p.hairColor ?? 0, totalWorkouts: p.totalWorkouts || 0, currentStreak: p.currentStreak || 0, muscleLevels: p.muscleLevels || {}, you: p.you }));
