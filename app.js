...
const COSMETICS = [
  ...
  { id: 'raid_trophy',     name: 'Raid Trophy',    icon: '🏆', type: 'neck',  unlock: 'raid_slayer',   desc: 'Defeat a weekly raid boss' },
...
const ACHIEVEMENTS = [
  ...
  // ── Raid Boss ──
  { id: 'raid_slayer',   icon: '<i class="ti ti-sword"></i>',  name: 'Boss Slayer',   desc: 'Defeat a weekly raid boss', check: s => Object.keys(s.raidVictoryClaimed||{}).length >= 1, title: 'Boss Slayer' },
  { id: 'raid_champion', icon: '<i class="ti ti-crown"></i>',  name: 'Raid Champion', desc: 'Defeat 10 weekly raid bosses', check: s => Object.keys(s.raidVictoryClaimed||{}).length >= 10, title: 'The Slayer' },
];
...
// Weekly Raid Boss — one shared global HP pool, everyone's workouts contribute.
// Boss rotates automatically each week (deterministic from the week number, so every
// player sees the same boss with no server coordination needed). Each boss has a
// weakness — matching workouts deal double damage.
const RAID_BOSSES = [
  { id: 'ogre',        name: 'Twin-Skull Ogre', subtitle: 'Bonecrusher of the Wastes',   hp: 20000, weakness: 'chest',     img: 'assets/bosses/orgre.png',       atkMin: 5, atkMax: 16, parryChance: 0.15, aoe: false },
  { id: 'wizard',      name: 'Arcane Sorcerer', subtitle: 'Keeper of Forbidden Tomes',   hp: 14000, weakness: 'abs',       img: 'assets/bosses/wizard.png',      atkMin: 3, atkMax: 9,  parryChance: 0.10, aoe: true },
  { id: 'orc_warrior', name: 'Orc Warrior',     subtitle: 'Arena Berserker',             hp: 16000, weakness: 'back',      img: 'assets/bosses/orc_warrior.png', atkMin: 6, atkMax: 14, parryChance: 0.20, aoe: false },
  { id: 'troll',       name: 'Troll Spearman',  subtitle: 'Hunter of the Deep Caves',    hp: 18000, weakness: 'shoulders', img: 'assets/bosses/troll.png',       atkMin: 4, atkMax: 10, parryChance: 0.12, aoe: true },
];

// Role scaling for how much a party member's workout XP contributes to shared raid boss
// damage. Tanks and healers are support roles — DPS carries the damage.
...
function getRaidBoss() {
  ...
  const idx = ((weekNum % RAID_BOSSES.length) + RAID_BOSSES.length) % RAID_BOSSES.length;
  return { boss: RAID_BOSSES[idx], weekStart: ws };
}
...
function submitWorkout() {
  ...
  for (const p of parsed) {
    ...
    // Monthly Raid is retired — replaced by the Weekly Raid Boss below.

    // ─── Weekly Raid Boss (global shared HP pool, weakness deals 2x) ───
    const { boss: rBoss, weekStart: rWs } = getRaidBoss();
    if (state.raidWeekStart !== rWs) { state.raidWeekStart = rWs; state.raidDamage = 0; }
    const hitMuscles = Object.keys(EXERCISE_MUSCLES[p.exercise] || {});
    const isWeakness = hitMuscles.includes(rBoss.weakness) || p.muscleKey === rBoss.weakness;
    // Party role scales how hard this workout hits the boss — DPS carries damage,
    // tank/healer are support roles. Solo players (no team) hit at the standard rate.
    const roleMult = (state.teamId && state.teamRole) ? (ROLE_DAMAGE_MULT[state.teamRole] ?? 1) : 1;
    const thisRaidDmg = Math.floor(raidDmg * (isWeakness ? 2 : 1) * roleMult);
    state.raidDamage = (state.raidDamage || 0) + thisRaidDmg;
...
// ─── Weekly Raid Boss — DUEL-style fight screen ───
const RAID_VICTORY_XP = 400, RAID_VICTORY_GOLD = 150;

// One-time migration: Monthly Raid is retired and merged into the Weekly Raid Boss.
// Whatever damage a player had already dealt to this month's raid boss carries over
// as a head start against the new shared boss.
function _migrateMonthlyRaidToWeeklyBoss() {
  if (state._monthlyToRaidMigrated) return;
  const { weekStart } = getRaidBoss();
  if (state.raidWeekStart !== weekStart) { state.raidWeekStart = weekStart; state.raidDamage = 0; }
...
async function renderRaidFight() {
  ...
  if (!el) return;
  _migrateMonthlyRaidToWeeklyBoss();
  const { boss, weekStart } = getRaidBoss();
  if (state.raidWeekStart !== weekStart) { state.raidWeekStart = weekStart; state.raidDamage = 0; }

  let globalDamage = state.raidDamage || 0;
  if (db) {
    try {
      const snap = await db.collection('users').where('_raidWeekStart', '==', weekStart).get();
      let total = 0;
      snap.forEach(doc => { total += doc.data()._raidDamage || 0; });
      if (total > 0) globalDamage = Math.max(total, globalDamage);
    } catch (e) {}
  }

  const pct = Math.min(100, (globalDamage / boss.hp) * 100);
  const beaten = pct >= 100;
  const hpLeft = Math.max(0, boss.hp - globalDamage);

  // One-time victory reward per player per week
  if (beaten && !state.raidVictoryClaimed?.[weekStart]) {
    if (!state.raidVictoryClaimed) state.raidVictoryClaimed = {};
    state.raidVictoryClaimed[weekStart] = true;
  ...
  // If you're in a party, show its live action log instead of the generic flavor line
  let partyLogEntry = null;
  if (db && state.teamId && !beaten) {
    try {
      const tDoc = await db.collection('teams').doc(state.teamId).get();
      const entries = tDoc.exists ? (tDoc.data().actionLog || []) : [];
      if (entries.length) partyLogEntry = entries[entries.length - 1].text;
    } catch (e) {}
  }
  ...
  el.innerHTML = `
    <div style="background:radial-gradient(ellipse at 50% 0%,#1a1030 0%,#0a0a12 60%),linear-gradient(90deg,#1a1030 0%,#0a0a12 48%,#0a0a12 52%,#1f0e14 100%);padding:18px 14px 16px;position:relative;overflow:hidden">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:2px">
        ...
        <span style="margin-left:auto;font-size:11px;color:#ccc;font-family:monospace">${Math.round(hpLeft).toLocaleString()}/${boss.hp.toLocaleString()}</span>
      </div>
      ...
      <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:22px;position:relative">
        <div style="text-align:center;flex:1;max-width:130px">
          <div style="position:relative;height:96px;display:flex;align-items:flex-end;justify-content:center">
            <div style="position:absolute;bottom:2px;width:76px;height:22px;border-radius:50%;background:radial-gradient(ellipse,rgba(158,124,255,0.45) 0%,transparent 72%);filter:blur(1px)"></div>
            <div class="duel-fighter-art" style="width:78px;height:88px;filter:drop-shadow(0 0 10px rgba(158,124,255,0.55))">${playerAvatar}</div>
          </div>
      ...
      <div style="font-size:10px;color:#666;text-align:center;margin-top:10px">Global HP pool — every player's workouts count. Resets Monday.</div>
    </div>`;
}
...
async function _maybeResetMonthlyHP(teamId, team) {
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      const log = _teamLogPush(t.actionLog || [], `🌙 A new month begins — the whole party is fully healed!`);
      tx.update(ref, { members, actionLog: log, lastHpResetMonth: thisMonth });
    });
  } catch (e) {}
}
...
async function _maybeTriggerDailyBossAttack(teamId, team) {
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      if (!doc.exists) return;
      const t = doc.data();
      if (t.lastBossAttackDate === today) return; // someone else already resolved it
...
// and either way a damage/heal line gets appended to the party's shared action log.
async function _applyTeamWorkoutEffects(raidContribution, sessionXP) {
  ...
  await db.runTransaction(async tx => {
    ...
    if (state.teamRole === 'healer') {
      const heal = Math.max(1, Math.floor(sessionXP * HEAL_FACTOR));
      for (const m of members) m.hp = Math.min(m.maxHp, m.hp + heal);
      log = _teamLogPush(log, `💚 ${myName}'s workout healed the party for ${heal} HP!`);
    } else if (raidContribution > 0) {
      const verb = state.teamRole === 'tank' ? 'tanked and dealt' : 'dealt';
      log = _teamLogPush(log, `⚡ ${myName} ${verb} ${raidContribution} damage to ${getRaidBoss().boss.name}!`);
    }
...
async function renderParty() {
  ...
  if (state.teamId) {
    ...
    if (!teamDoc || !teamDoc.exists) {
      ...
      renderPartyCard(el, finalTeam.name, finalMembers, finalTeam.ownerUsername, finalTeam.actionLog || []);
      return;
    }
  }
  ...
  const invitesHTML = invites.map(t => {
    const taken = new Set((t.members || []).map(m => m.role));
    const roleOptions = Object.entries(TEAM_ROLES).filter(([k, r]) => (t.members || []).filter(m => m.role === k).length < r.slots)
    ...
    return `<div style="border:1px solid var(--accent-glow,#9e7cff);border-radius:10px;padding:12px;margin-bottom:10px;background:rgba(158,124,255,0.08)">
      <div style="font-size:13px;font-weight:600;color:var(--text);margin-bottom:8px">Invited to <span style="color:var(--accent-glow,#9e7cff)">${esc(t.name)}</span></div>
...
function renderPartyCard(el, teamName, members, ownerUsername, actionLog) {
  const slots = [];
  for (const [roleKey, roleInfo] of Object.entries(TEAM_ROLES)) {
  ...
  const fixRoleHTML = myBrokenEntry ? `
    <div style="border:2px solid #ffc107;border-radius:8px;background:#ffc10714;padding:12px;margin-bottom:12px">
      ...
      <div style="display:flex;gap:6px">
        ${Object.entries(TEAM_ROLES).filter(([k, r]) => (members||[]).filter(m => m.role === k).length < r.slots).map(([k, r]) =>
  ...
  el.innerHTML = `
    <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:10px;text-align:center">${esc(teamName)}</div>
    ${fixRoleHTML}
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">${cardsHTML}</div>
    ${logHTML}
...
async function inviteToTeam() {
  ...
  const username = document.getElementById('team-invite-input')?.value?.trim().toLowerCase();
  ...
  try {
    ...
    if (!doc.exists) return;
    const team = doc.data();
    if ((team.members || []).length >= 4) { toast('Team is full'); return; }
    ...
    document.getElementById('team-invite-input').value = '';
    renderParty();
...
return {
  ...
  renderMiniAvatarSVG, getHairSVG, _buildBackAvatarSVG,
};
