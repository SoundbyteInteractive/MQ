...
async function loadHealthSync() {
  ...
  try {
    const doc = await db.collection('healthSync').doc(currentUser).get();
    if (doc.exists) {
      healthSync = doc.data();
      renderHealthPanel();
    }
...
function getHealthSyncSetupHTML() {
  const url = `https://firestore.googleapis.com/v1/projects/musclequest-c5052/databases/(default)/documents/healthSync/${currentUser || 'YOUR_USERNAME'}?key=AIzaSyC6_AVDVbH9cBh304PJOA7cMsTBcyTw5Rc`;
  return `<div class="health-setup">
    <p class="health-setup-intro">Link your iPhone's Apple Health &amp; Lose It! data to earn daily bonuses.</p>
    <div class="health-setup-steps">
      <div class="setup-step"><span class="step-num">1</span><span>Open the <strong>Shortcuts</strong> app on iPhone</span></div>
      <div class="setup-step"><span class="step-num">2</span><span>Tap <strong>+</strong> → <strong>Add Action</strong></span></div>
      <div class="setup-step"><span class="step-num">3</span><span>Search <strong>"Find Health Samples"</strong> — add it 4 times for: <em>Steps, Active Energy Burned, Dietary Protein, Sleep Analysis</em></span></div>
      <div class="setup-step"><span class="step-num">4</span><span>After each, add <strong>"Calculate Statistics"</strong> → Sum, rename each result: <em>mySteps, myActiveCal, myProtein, mySleep</em></span></div>
      <div class="setup-step"><span class="step-num">5</span><span>For Sleep: add <strong>"Calculate"</strong> → divide mySleep by 3600 → rename <em>mySleepHours</em></span></div>
      <div class="setup-step"><span class="step-num">6</span><span>Add <strong>"Format Date"</strong> → Current Date → Custom Format: <code>YYYY-MM-DD</code> → rename <em>myDate</em></span></div>
      <div class="setup-step"><span class="step-num">7</span><span>Add <strong>"Text"</strong> action and paste this (insert your variables using the magic variable picker):<br>
        <code class="setup-code">{"fields":{"date":{"stringValue":"myDate"},"steps":{"doubleValue":mySteps},"protein":{"doubleValue":myProtein},"activeCalories":{"doubleValue":myActiveCal},"sleepHours":{"doubleValue":mySleepHours},"exerciseMinutes":{"doubleValue":myExerciseMinutes}}}</code>
      </span></div>
      <div class="setup-step"><span class="step-num">8</span><span>Add <strong>"Get Contents of URL"</strong>:<br>
        • Method: <strong>PATCH</strong><br>
        • Headers: <code>Content-Type: application/json</code><br>
        • Body: <strong>Request Body → JSON → paste Text from step 7</strong>
      </span></div>
      <div class="setup-step"><span class="step-num">9</span><span>Set the URL to your personal sync link (tap to copy):</span></div>
    </div>
    <div class="setup-url-box" onclick="navigator.clipboard.writeText('${url}');MQ.toast('Copied!')">
      <code class="setup-url">${url}</code>
      <span class="setup-copy"><i class="ti ti-copy"></i></span>
    </div>
    <div class="setup-step" style="margin-top:10px"><span class="step-num">10</span><span>Tap <strong>Done</strong>, name it <strong>"MuscleQuest Sync"</strong>. Run it once to grant Health permissions. Then go to <strong>Automations → +</strong> and set it to run daily at a time of your choice.</span></div>
    <div class="health-bonuses-info">
      <strong>Daily bonuses unlocked by syncing:</strong>
    ...
    ${healthSync ? `<div class="health-last-sync">Last sync: <strong>${healthSync.date}</strong> ${healthSync.date === todayStr() ? '✓ Today' : ''}</div>` : ''}
  </div>`;
}
...
function renderAvatar() {
  ...
  const headHTML = front ? `
    ...
    <g class="head-group" style="transform-origin: 100px 95px;">
      ...
      ${female ? `
      <ellipse cx="100" cy="63" rx="25" ry="17" fill="${hairColor}"/>
      <path d="M78 60 Q100 54 122 60" stroke="${hairDark}" stroke-width="1" fill="none"/>
      <circle cx="74" cy="55" r="13" fill="${hairColor}"/>
      <path d="M60 50 Q74 46 88 50" stroke="${hairDark}" stroke-width="2" fill="none"/>
      <path d="M60 55 Q74 51 88 55" stroke="${hairDark}" stroke-width="1.2" fill="none"/>
      <path d="M60 60 Q53 74 57 88" stroke="${hairColor}" stroke-width="3.5" fill="none" stroke-linecap="round"/>
      <circle cx="126" cy="55" r="13" fill="${hairColor}"/>
      <path d="M112 50 Q126 46 140 50" stroke="${hairDark}" stroke-width="2" fill="none"/>
      <path d="M112 55 Q126 51 140 55" stroke="${hairDark}" stroke-width="1.2" fill="none"/>
      <path d="M140 60 Q147 74 143 88" stroke="${hairColor}" stroke-width="3.5" fill="none" stroke-linecap="round"/>
...
// ─── Leaderboard ───
function renderMiniAvatarSVG(gender, cosmetics, idx, playerName) {
  const female = gender === 'female';
  const hc = '#4a3728', hd = '#3d2e1f';
  const eq = cosmetics || [];
  const skin = '#e8b47a', skinD = '#c8965e';
  ...
  const wristSVG = wristColor ? `<rect x="${lwx}" y="${lwy}" width="6" height="3" rx="1" fill="${wristColor}"/>
    <rect x="${rwx}" y="${rwy}" width="6" height="3" rx="1" fill="${wristColor}"/>` : '';
  ...
  // Female: curtain bangs (part in middle, sweep outward)
  const bangsSVG = female ? `
    <path d="M50 16 Q43 20 38 25 Q39 29 42 29 Q46 24 50 22Z" fill="${hc}"/>
    <path d="M50 16 Q57 20 62 25 Q61 29 58 29 Q54 24 50 22Z" fill="${hc}"/>
    <path d="M50 16 Q44 21 40 25" stroke="${hd}" stroke-width="0.7" fill="none"/>
    <path d="M50 16 Q56 21 60 25" stroke="${hd}" stroke-width="0.7" fill="none"/>` : '';
  // Female: thicker side curtains; male: nothing (arms cover sides)
  const sideHairSVG = female ? `
    <path d="M38 18 Q34 30 35 44" stroke="${hc}" stroke-width="4" fill="none" stroke-linecap="round"/>
    <path d="M62 18 Q66 30 65 44" stroke="${hc}" stroke-width="4" fill="none" stroke-linecap="round"/>` : '';
  // Female: full hair cap + space buns (drawn after head so they sit on top)
  const femaleTopHairSVG = female ? `
    <ellipse cx="50" cy="13" rx="14" ry="9" fill="${hc}"/>
    <path d="M39 11 Q50 7 61 11" stroke="${hd}" stroke-width="0.8" fill="none"/>
    <circle cx="36" cy="9" r="7" fill="${hc}"/>
    <path d="M29 6 Q36 4 43 6" stroke="${hd}" stroke-width="1.2" fill="none"/>
    <path d="M29 9 Q36 7 43 9" stroke="${hd}" stroke-width="0.7" fill="none"/>
    <path d="M29 12 Q26 18 28 26" stroke="${hc}" stroke-width="1.8" fill="none" stroke-linecap="round"/>
    <circle cx="64" cy="9" r="7" fill="${hc}"/>
    <path d="M57 6 Q64 4 71 6" stroke="${hd}" stroke-width="1.2" fill="none"/>
    <path d="M57 9 Q64 7 71 9" stroke="${hd}" stroke-width="0.7" fill="none"/>
    <path d="M71 12 Q74 18 72 26" stroke="${hc}" stroke-width="1.8" fill="none" stroke-linecap="round"/>` : '';
  ...
  return `<svg viewBox="0 0 100 110" xmlns="http://www.w3.org/2000/svg" class="mini-avatar">
    ...
    ${female ? femaleTopHairSVG : `<path d="M37 18 Q41 7 50 6 Q59 7 63 18 Q61 11 50 10 Q39 11 37 18Z" fill="${hc}"/>`}
    ${bangsSVG}
...
async function renderLeaderboard(type) {
  ...
  if (type === 'bodybuilding') {
    await renderBBLeaderboard(container);
    return;
  }
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        players.push({
          ...
          lastActive: d._lastActive || '',
        });
      });

      if (type === 'weekly') {
        const weekStart = getWeekStart();
        players = players.filter(p => p.lastActive >= weekStart || p.you);
  ...
  if (!players) {
    players = [
      { name: state.name, xp: myXP, level: overall.level, you: true, gender: state.gender, cosmetics: state.equippedCosmetics },
    ];
  }

  // Ensure "you" always has your current cosmetics
  players = players.map(p => p.you ? { ...p, gender: state.gender, cosmetics: state.equippedCosmetics } : p);
  ...
  podium.innerHTML = `<div class="lb-podium">
    ${[1, 0, 2].map(pos => {
      ...
      return `<div class="podium-slot rank-${rank}${p.you ? ' podium-you' : ''}">
        <div class="podium-avatar">${renderMiniAvatarSVG(p.gender || 'male', p.cosmetics || [], pos, p.name || '')}</div>
...
async function renderFightLeaderboard(container) {
  ...
  if (db) {
    try {
      ...
      if (!hasMe) {
        fighters.push({ name: state.name, kos: myKOs, highest: myHighest, streak: myStreak, you: true, gender: state.gender, cosmetics: state.equippedCosmetics });
      }
  ...
  if (!fighters) {
    fighters = [
      { name: state.name, kos: myKOs, highest: myHighest, streak: myStreak, you: true, gender: state.gender, cosmetics: state.equippedCosmetics },
    ];
  }

  fighters = fighters.map(f => f.you ? { ...f, gender: state.gender, cosmetics: state.equippedCosmetics } : f);
...
async function renderBBLeaderboard(container) {
  ...
  if (db) {
    try {
      ...
      const hasMe = players.some(p => p.you);
      if (!hasMe) {
        players.push({ name: state.name, wins: myWins, comps: myComps, streak: myStreak, you: true, gender: state.gender, cosmetics: state.equippedCosmetics });
      }
  ...
  if (!players) {
    players = [
      { name: state.name, wins: myWins, comps: myComps, streak: myStreak, you: true, gender: state.gender, cosmetics: state.equippedCosmetics },
    ];
  }
...
return {
  ...
  login, logout, toggleCosmetic, selectPose, toggleSettings, toast, syncStateFromStorage,
};
