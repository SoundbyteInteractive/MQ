...
// Monthly Raid Boss — one shared global HP pool, everyone's workouts contribute.
// Boss rotates automatically each month (deterministic from the month index, so every
...
function submitWorkout() {
  ...
  for (const p of parsed) {
    ...
    // ─── Monthly Raid Boss (global shared HP pool, weakness deals 2x) ───
...
async function renderRaidFight() {
  ...
  el.innerHTML = `
    <div style="background:radial-gradient(ellipse at 50% 0%,#1a1030 0%,#0a0a12 60%),linear-gradient(90deg,#1a1030 0%,#0a0a12 48%,#0a0a12 52%,#1f0e14 100%);padding:18px 14px 16px;position:relative;overflow:hidden">
      ...
      <div style="font-size:10px;color:#666;text-align:center;margin-top:10px">Global HP pool — every player's workouts count. Resets monthly.</div>
    </div>`;
}
