...
async function renderLeaderboard(type) {
  ...
  players = players.map(p => p.you ? { ...p, xp: myPeriodXP, gender: state.gender, cosmetics: state.equippedCosmetics, hair: state.hair, skinTone: state.skinTone ?? 1, hairColor: state.hairColor ?? 0, totalWorkouts: state.totalWorkouts || 0, currentStreak: state.streak || 0, muscleLevels: myMuscleLevels } : p);
  players.sort((a, b) => b.xp - a.xp);
