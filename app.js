...
function submitWorkout() {
  ...
  for (const p of parsed) {
    ...
    if (p.isRest) {
      totalXP = Math.floor((hasBuff('bcaa') ? 150 : 100) * vialMult);
...
function renderAvatar() {
  ...
  // Tattoo — compute before headHTML so face tat can go inside head group
  ...
  // Body tats: scale(2,2.9) maps mini body coords to main avatar 200x320 space
  const _bodyTatSVG = (_activeTat && _activeTat.placement !== 'face')
    ? `<g transform="scale(2,2.9)" opacity="0.88">${_activeTat.svg}</g>` : '';
...
function showPRGraph(exercise, muscle) {
  ...
  modal.innerHTML = `
    <div style="background:var(--card-bg);border-radius:18px;width:100%;max-width:400px;padding:22px 22px 28px;box-shadow:0 8px 48px rgba(0,0,0,0.6)">
...
function renderVolumeReport(period) {
  ...
  return `
    ...
    <div style="font-size:13px;color:var(--muted);text-align:center;margin-bottom:14px">${totalSets} total sets · last ${days} days</div>
...
function renderVolumeBody(muscleSets) {
  const maxS = 20;
  const c = k => MUSCLES[k]?.color || '#888';
  const op = k => muscleSets[k] ? Math.min(0.88, 0.2 + (muscleSets[k] / maxS) * 0.68) : 0;
  const bg = '#1e1c2a', line = '#3a384a';
  const ox = 108;

  const bodyBase = (xOff) => `
    <ellipse cx="${xOff+40}" cy="11" rx="11" ry="12" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <rect x="${xOff+36}" y="22" width="8" height="7" fill="${bg}"/>
    <path d="M${xOff+20},32 L${xOff+60},32 L${xOff+62},86 L${xOff+18},86 Z" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <path d="M${xOff+20},33 L${xOff+10},37 L${xOff+9},74 L${xOff+18},74 Z" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <path d="M${xOff+60},33 L${xOff+70},37 L${xOff+71},74 L${xOff+62},74 Z" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <rect x="${xOff+22}" y="86" width="15" height="42" rx="4" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <rect x="${xOff+43}" y="86" width="15" height="42" rx="4" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <rect x="${xOff+23}" y="126" width="13" height="28" rx="3" fill="${bg}" stroke="${line}" stroke-width="0.8"/>
    <rect x="${xOff+44}" y="126" width="13" height="28" rx="3" fill="${bg}" stroke="${line}" stroke-width="0.8"/>`;

  const frontOverlays = (xOff) => `
    ${op('shoulders') > 0 ? `<ellipse cx="${xOff+16}" cy="37" rx="8" ry="6" fill="${c('shoulders')}" opacity="${op('shoulders').toFixed(2)}"/>
    <ellipse cx="${xOff+64}" cy="37" rx="8" ry="6" fill="${c('shoulders')}" opacity="${op('shoulders').toFixed(2)}"/>` : ''}
    ${op('chest') > 0 ? `<rect x="${xOff+23}" y="33" width="34" height="22" rx="3" fill="${c('chest')}" opacity="${op('chest').toFixed(2)}"/>` : ''}
    ${op('biceps') > 0 ? `<ellipse cx="${xOff+12}" cy="56" rx="5" ry="9" fill="${c('biceps')}" opacity="${op('biceps').toFixed(2)}"/>
    <ellipse cx="${xOff+68}" cy="56" rx="5" ry="9" fill="${c('biceps')}" opacity="${op('biceps').toFixed(2)}"/>` : ''}
    ${op('abs') > 0 ? `<rect x="${xOff+27}" y="55" width="26" height="28" rx="3" fill="${c('abs')}" opacity="${op('abs').toFixed(2)}"/>` : ''}
    ${op('quads') > 0 ? `<rect x="${xOff+23}" y="88" width="13" height="34" rx="3" fill="${c('quads')}" opacity="${op('quads').toFixed(2)}"/>
    <rect x="${xOff+44}" y="88" width="13" height="34" rx="3" fill="${c('quads')}" opacity="${op('quads').toFixed(2)}"/>` : ''}
    ${op('calves') > 0 ? `<rect x="${xOff+24}" y="128" width="11" height="24" rx="3" fill="${c('calves')}" opacity="${op('calves').toFixed(2)}"/>
    <rect x="${xOff+45}" y="128" width="11" height="24" rx="3" fill="${c('calves')}" opacity="${op('calves').toFixed(2)}"/>` : ''}`;

  const backOverlays = (xOff) => `
    ${op('back') > 0 ? `<path d="M${xOff+22},33 L${xOff+58},33 L${xOff+60},65 L${xOff+20},65 Z" fill="${c('back')}" opacity="${op('back').toFixed(2)}"/>` : ''}
    ${op('triceps') > 0 ? `<ellipse cx="${xOff+11}" cy="56" rx="4" ry="8" fill="${c('triceps')}" opacity="${op('triceps').toFixed(2)}"/>
    <ellipse cx="${xOff+69}" cy="56" rx="4" ry="8" fill="${c('triceps')}" opacity="${op('triceps').toFixed(2)}"/>` : ''}
    ${op('glutes') > 0 ? `<rect x="${xOff+23}" y="87" width="14" height="14" rx="4" fill="${c('glutes')}" opacity="${op('glutes').toFixed(2)}"/>
    <rect x="${xOff+43}" y="87" width="14" height="14" rx="4" fill="${c('glutes')}" opacity="${op('glutes').toFixed(2)}"/>` : ''}
    ${op('hamstrings') > 0 ? `<rect x="${xOff+23}" y="100" width="13" height="22" rx="3" fill="${c('hamstrings')}" opacity="${op('hamstrings').toFixed(2)}"/>
    <rect x="${xOff+44}" y="100" width="13" height="22" rx="3" fill="${c('hamstrings')}" opacity="${op('hamstrings').toFixed(2)}"/>` : ''}
    ${op('calves') > 0 ? `<rect x="${xOff+24}" y="128" width="11" height="24" rx="3" fill="${c('calves')}" opacity="${op('calves').toFixed(2)}"/>
    <rect x="${xOff+45}" y="128" width="11" height="24" rx="3" fill="${c('calves')}" opacity="${op('calves').toFixed(2)}"/>` : ''}`;

  return `<svg viewBox="0 0 ${ox + 80} 168" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:300px">
    ${bodyBase(0)}${frontOverlays(0)}
    <text x="40" y="162" text-anchor="middle" font-size="8" fill="#666" font-family="sans-serif">FRONT</text>
    ${bodyBase(ox)}${backOverlays(ox)}
    <text x="${ox+40}" y="162" text-anchor="middle" font-size="8" fill="#666" font-family="sans-serif">BACK</text>
  </svg>`;
}
...
return {
  ...
  showVolume, adoptLegendSplit, deleteSplit, openSplitBuilder, _saveSplitBuilder, _sbAddDay, _sbRemoveDay, _sbChangePreset, _sbToggleEx, renderSettingsSplits, cycleSplitDay, shareSplit, importSplitCode, _showImportCode,
